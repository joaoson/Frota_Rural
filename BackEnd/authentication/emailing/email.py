...
## Código de uso único enviado antes do aceite do contrato. Serve como prova
## de posse do endereço de e-mail no registro de assinatura eletrônica.
def send_contract_signature_otp_email(to_email: str, code: str, contract_number: str, timeout: int) -> None:
    resend.api_key = settings.RESEND_API_KEY
    from_email = settings.RESEND_SUPPORT_EMAIL
    ...
    params: resend.Emails.SendParams = {
        "from": from_email,
        "to": [to_email],
        "subject": f"Código para assinar o contrato {contract_number}",
        "html": _build_contract_otp_email_html(code, contract_number, timeout),
    }

    resend.Emails.send(params)


def _build_contract_otp_email_html(code: str, contract_number: str, timeout: int) -> str:
    return f"""<!DOCTYPE html>
<html lang="pt-BR">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Código de Assinatura</title>
</head>
<body style="margin:0;padding:0;background-color:#fffaeb;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background-color:#fffaeb;">
    <tr>
        ...
        <table width="100%" cellpadding="0" cellspacing="0" role="presentation"
          ...
          <tr>
            <td style="padding:24px 40px 36px;">
              ...
              <p style="margin:0 0 24px;font-size:15px;line-height:1.75;color:#3f4336;">
                Use o código abaixo para confirmar a assinatura do contrato
                <strong style="color:#143d0e;">{contract_number}</strong> na
                <strong style="color:#143d0e;">Frota Rural</strong>.
              </p>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
                <tr>
                  <td align="center" style="padding-bottom:32px;">
                    <div style="display:inline-block;padding:18px 40px;background-color:#f5f7f0;border:1px solid #c3c8b5;border-radius:10px;font-size:34px;font-weight:800;color:#143d0e;letter-spacing:10px;">
                      {code}
                    </div>
                  </td>
                </tr>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
                <tr>
                  <td style="background-color:#fff8e8;border:1px solid #fed566;border-radius:8px;padding:12px 16px;">
                    <p style="margin:0;font-size:13px;color:#7a5c00;line-height:1.5;">
                      &#9200; Este código expira em <strong>{timeout} minutos</strong> e só pode ser usado uma vez.
                    </p>
                  </td>
                </tr>
              </table>
          ...
          <tr>
            <td style="background-color:#f5f0e8;border-top:1px solid #e8e3d8;border-radius:0 0 20px 20px;padding:20px 40px;">
              <p style="margin:0 0 6px;font-size:12px;color:#6b7165;line-height:1.6;">
                Se você não está assinando este contrato, ignore este e-mail e não
                compartilhe o código com ninguém.
              </p>
              <p style="margin:0;font-size:11px;color:#9aa090;">
                &copy; 2026 Frota Rural
              </p>
