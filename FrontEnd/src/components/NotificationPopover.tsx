...
const NotificationPopover = ({ notifications }: NotificationPopoverProps) => {
  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen(!open)} className="relative text-on-surface-variant hover:text-primary dark:hover:text-primary-bright transition-colors">
      ...
      {open ? (
        <div className="absolute right-0 top-full mt-2 w-96 bg-surface-container-lowest border border-outline-variant/30 rounded-2xl shadow-xl z-50 overflow-hidden">
          <div className="p-4 border-b border-outline-variant/30 flex items-center justify-between">
            ...
            <span className="text-xs font-bold text-primary dark:text-primary-bright">{unreadCount} novas</span>
          </div>
            ...
            {notifications.map((n) => (
              ...
              >
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${n.unread ? "bg-primary/10" : "bg-surface-container-high"}`}>
                  <MaterialIcon icon={n.icon} className={n.unread ? "text-primary dark:text-primary-bright" : "text-on-surface-variant"} size={20} />
                </div>
          ...
          <div className="p-3 border-t border-outline-variant/30">
            <button className="w-full text-center text-sm font-bold text-primary dark:text-primary-bright hover:underline py-1">Ver todas as notificações</button>
          </div>
        </div>
      ) : null}
    </div>
