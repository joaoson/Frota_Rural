...
class PricingService {
  private SUGGEST_ENDPOINT = "pricing/suggest";

  /**
   * Pede uma sugestão de valor/hora para a máquina.
   *
   * Responde 422 quando não há dado de mercado confiável — situação normal, não
   * falha: o locador simplesmente digita o valor. Quem chama deve tratar 422
   * como "sem sugestão", com a mensagem do backend, e não como erro.
   */
  async suggest(payload: SuggestPricePayload) {
    const response = await AxiosInstance.post<PricingSuggestion>(
      this.SUGGEST_ENDPOINT,
      payload,
    );
