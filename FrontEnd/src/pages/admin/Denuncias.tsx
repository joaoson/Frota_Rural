...
import type { FlaggedMessage } from "@/features/administration/types/moderation";
...
export default function Denuncias() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setRows(await moderationStore.listFlaggedMessages("pending"));
    } catch (error) {
  ...
  const decide = async (id: string, decision: "dismiss" | "hide") => {
    try {
      await moderationStore.resolveFlaggedMessage(id, decision);
        ...
        <div className="overflow-x-auto rounded-2xl border border-outline-variant/30">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-outline-variant/30">
              {rows.map((row) => (
                <tr key={row.messageId} className="align-top">
                  ...
                  <td className="p-3">
                    <div className="flex gap-2">
                      <Button size="sm" variant="outline" onClick={() => decide(row.messageId, "dismiss")}>
                        Arquivar
                      </Button>
                      <Button size="sm" onClick={() => decide(row.messageId, "hide")}>
                        Ocultar
                      </Button>
                    </div>
                  </td>
