...
/** A entidade traz Date; os cálculos da página trabalham com YYYY-MM-DD. */
const iso = (value: Date | null) => value?.toISOString().slice(0, 10) ?? "";
...
const AnaliseLocacao = () => {
  ...
  const abrirConversa = async () => {
    ...
    setAbrindoChat(true);
    try {
      const thread = await chatStore.resolveThread("rental", rentalId);
  ...
  useEffect(() => {
    if (!rentalId) return;
    contractStore.findRentalById(rentalId).then(setRental).catch(() => setRental(null)).finally(() => setLoading(false));
  }, [rentalId]);

  const analytics = useMemo(() => {
    if (!rental) return null;
    const totalDays = daysBetween(iso(rental.startDate), iso(rental.endDate));
    const today = new Date(); today.setHours(12, 0, 0, 0);
    const started = Math.max(0, Math.min(totalDays, Math.floor((today.getTime() - toDate(iso(rental.startDate)).getTime()) / 86_400_000) + 1));
    ...
    // A entidade já entrega número; formatar é papel da apresentação.
    const total = rental.totalPrice ?? 0;
  ...
  const extensionStart = isoDate(new Date(toDate(iso(rental.endDate)).getTime() + 86_400_000));
  ...
  return <main className="min-h-screen bg-background p-5 md:p-10">
    <div className="mx-auto max-w-6xl space-y-7">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between"><div><Link to="/dashboard-locatario" className="text-sm font-bold text-primary hover:underline">← Voltar para minhas locações</Link><h1 className="mt-3 font-headline text-3xl font-black text-primary">Análise da locação</h1><p className="mt-2 text-on-surface-variant">{[rental.machineBrand, rental.machineModel].filter(Boolean).join(" ")} · Contrato {rental.contractNumber}</p></div><span className="self-start rounded-full bg-primary/10 px-4 py-2 text-sm font-bold text-primary">{statusLabel}</span></div>

      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[{ icon: "calendar_month", label: "Período contratado", value: `${analytics.totalDays} dias`, detail: `${formatDate(iso(rental.startDate))} — ${formatDate(iso(rental.endDate))}` }, { icon: "event_available", label: "Dias utilizados", value: `${analytics.usedDays} dias`, detail: rental.status === "active" ? `${analytics.remainingDays} restantes` : "Período finalizado" }, { icon: "payments", label: "Valor contratado", value: rental.totalPrice, detail: `${analytics.daily.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}/dia` }, { icon: "description", label: "Contrato", value: rental.contractNumber, detail: statusLabel }].map((item) => <div key={item.label} className="rounded-2xl border border-outline-variant/30 bg-surface-container-lowest p-5 shadow-sm"><MaterialIcon icon={item.icon} className="text-primary" size={22} /><p className="mt-4 text-[11px] font-bold uppercase tracking-wider text-on-surface-variant">{item.label}</p><p className="mt-1 text-xl font-black text-on-surface">{item.value}</p><p className="mt-1 text-xs text-on-surface-variant">{item.detail}</p></div>)}
      </section>
