...
import EditEquipamentoModal, { type EquipamentoData } from "@/components/EditEquipamentoModal";
...
const mockMachines = [
  { id: 1, renagro: "BR1029304899", brand: "John Deere", model: "S700", year: 2022, status: "active", purpose: "Colheita" },
  { id: 2, renagro: "BR5048201734", brand: "Valtra", model: "BH194", year: 2021, status: "active", purpose: "Preparo de Solo" },
  { id: 3, renagro: "BR7012345678", brand: "Massey Ferguson", model: "7700", year: 2019, status: "removed", purpose: "Plantio" },
  { id: 4, renagro: "BR3098712345", brand: "New Holland", model: "CR7.90", year: 2023, status: "active", purpose: "Colheita" },
  { id: 5, renagro: "BR6054389012", brand: "Case IH", model: "Magnum 380", year: 2020, status: "active", purpose: "Preparo de Solo" },
  { id: 6, renagro: "BR4021567890", brand: "Jacto", model: "Uniport 4530", year: 2022, status: "active", purpose: "Pulverização" },
];

const mockPostings = [
  { id: 1, machine: "JD S700", price: 480, location: "Sorriso, MT", status: "active" },
  { id: 2, machine: "Valtra BH194", price: 320, location: "Lucas do Rio Verde, MT", status: "active" },
  { id: 3, machine: "New Holland CR7.90", price: 520, location: "Sinop, MT", status: "active" },
  { id: 4, machine: "Case IH Magnum", price: 380, location: "Rondonópolis, MT", status: "active" },
  { id: 5, machine: "Jacto Uniport", price: 290, location: "Primavera do Leste, MT", status: "active" },
];

const mockRentals = [
  { id: 1, lessee: "Fazenda Aurora", machine: "Trator Valtra BH194", period: "02 – 10 Fev/2026", status: "pending", total: "R$ 15.000,00", contract: "#CTR-8821" },
  { id: 2, lessee: "Fazenda São João", machine: "Colheitadeira JD S700", period: "15 – 20 Jan/2026", status: "active", total: "R$ 38.400,00", contract: "#CTR-8799" },
  { id: 3, lessee: "Fazenda Boa Vista", machine: "Pulverizador Jacto", period: "01 – 05 Dez/2025", status: "completed", total: "R$ 6.250,00", contract: "#CTR-8745" },
  { id: 4, lessee: "Fazenda Esperança", machine: "Trator Massey 7700", period: "20 – 28 Nov/2025", status: "cancelled", total: "R$ 12.000,00", contract: "#CTR-8710" },
];
...
const DashboardLocador = () => {
  ...
  const [selectedEquipamento, setSelectedEquipamento] = useState<EquipamentoData>({
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "pending":
        return {
          icon: "description",
          classes: "bg-secondary-container/20 text-on-secondary-container border border-secondary-container/30",
          label: "Assinatura Pendente",
        };
      case "active":
        return { icon: "circle", classes: "bg-primary/10 text-primary border border-primary/20", label: "Em Operação (Ativo)" };
      case "completed":
        return { icon: "check_circle", classes: "bg-primary/10 text-primary border border-primary/20", label: "Concluída" };
      case "cancelled":
        return { icon: "warning", classes: "bg-error/10 text-error border border-error/20", label: "Locação Cancelada" };
      case "validating":
        return {
          icon: "hourglass_bottom",
          classes: "bg-surface-container-high text-on-surface-variant border border-outline-variant/30",
  ...
  const paginatedFrota = machines.slice((frotaPage - 1) * frotaPerPage, frotaPage * frotaPerPage);
  const paginatedAnuncios = mockPostings.slice((anunciosPage - 1) * anunciosPerPage, anunciosPage * anunciosPerPage);
      ...
      >
        ...
        <div className="ml-4 p-6">
          ...
          <div className="grid grid-cols-2 gap-3 bg-surface-container-lowest p-4 rounded-xl border border-outline-variant/20 mb-4">
            <div>
              <div className="text-[10px] font-bold text-outline uppercase tracking-wider mb-1">Locatário</div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {r.status === "completed" ? (
              <button
                onClick={() => setShowAvaliar(showAvaliar === r.id ? null : r.id)}
            ...
            {r.status === "completed" || r.status === "cancelled" ? (
              <button
                onClick={() => setShowDetalhes(showDetalhes === r.id ? null : r.id)}
          ...
          {showDetalhes === r.id ? (
            <div className="mt-4 bg-surface-container-lowest border border-outline-variant/20 rounded-xl p-5 space-y-3 animate-in fade-in">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  <span className="text-outline text-xs font-bold uppercase">Contrato:</span>{" "}
                  <span className="text-on-surface font-bold">{r.contract}</span>
                </div>
                <div>
                  <span className="text-outline text-xs font-bold uppercase">Locatário:</span>{" "}
                  <span className="text-on-surface font-bold">{r.lessee}</span>
                </div>
                <div>
                  <span className="text-outline text-xs font-bold uppercase">Maquinário:</span>{" "}
                  <span className="text-on-surface font-bold">{r.machine}</span>
                </div>
                <div>
                  <span className="text-outline text-xs font-bold uppercase">Período:</span>{" "}
                  <span className="text-on-surface font-bold">{r.period}</span>
                </div>
                <div>
                  <span className="text-outline text-xs font-bold uppercase">Valor Total:</span>{" "}
                  <span className="text-primary font-black">{r.total}</span>
                </div>
                <div>
                  <span className="text-outline text-xs font-bold uppercase">Status:</span>{" "}
          ...
          {showAvaliar === r.id ? (
            <div className="mt-4 bg-secondary-fixed/20 border border-secondary-container/30 rounded-xl p-5 space-y-4 animate-in fade-in">
              <h4 className="font-headline font-bold text-on-surface text-sm flex items-center gap-2">
                <MaterialIcon icon="star" size={16} className="text-on-secondary-container" /> Avaliar Locatário
              </h4>
              <p className="text-sm text-on-surface-variant">Como foi a experiência com {r.lessee}?</p>
    ...
    <div className="min-h-screen bg-background flex">
          ...
          {sidebarItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                tab === item.tab ? "bg-primary/10 text-primary font-bold border-l-2 border-primary" : "text-on-surface-variant hover:bg-surface-container-high"
              }`}
            >
      ...
      <main className="flex-1 min-w-0">
        <header className="h-16 border-b border-outline-variant/30 bg-surface-container-lowest/90 backdrop-blur-md flex items-center justify-between px-8 sticky top-0 z-10">
          ...
          <NotificationPopover
            notifications={[
              { id: 1, icon: "description", title: "Contrato aguardando assinatura", desc: "Fazenda Aurora · Trator Valtra BH194", time: "Agora", unread: true },
              { id: 2, icon: "event_available", title: "Nova solicitação de reserva", desc: "Fazenda São João · Colheitadeira JD S700", time: "2h atrás", unread: true },
              { id: 3, icon: "star", title: "Nova avaliação recebida", desc: "Fazenda Boa Vista avaliou com 5 estrelas", time: "Ontem", unread: false },
            ]}
          />
        </header>
        ...
        <div className="p-8 max-w-[1200px]">
                ...
                prev.map((m) =>
                  String(m.id) === data.id
                    ? {
                        ...
                        brand: data.marca,
                        model: data.modelo,
                        year: data.anoFabricacao ? Number(data.anoFabricacao) : m.year,
          ...
          {tab === "dashboard" ? (
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                  { label: "EQUIPAMENTOS", value: "3", sub: "na frota ativa", icon: "agriculture" },
                  { label: "ANÚNCIOS ATIVOS", value: "2", sub: "publicados", icon: "campaign", dotColor: "bg-primary" },
                  { label: "LOCAÇÕES ATIVAS", value: "1", sub: "em andamento", icon: "event_available" },
                  { label: "RECEITA NO MÊS", value: "R$ 4.800", sub: "fevereiro 2026", icon: "payments" },
                ].map((stat) => (
                  <div key={stat.label} className="bg-surface-container-low border border-outline-variant/30 rounded-2xl p-6 hover:shadow-md transition-all group">
                    ...
                    <div className="text-sm text-on-surface-variant flex items-center gap-1.5">
                      {stat.dotColor ? <span className={`w-2 h-2 rounded-full ${stat.dotColor} animate-pulse`} /> : null}
                      {stat.sub}
                    </div>
                  </div>
                ))}
              ...
              <div className="bg-surface-container-lowest border border-outline-variant/30 rounded-2xl p-6 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h2 className="font-headline text-lg font-bold text-on-surface">Evolução do Faturamento</h2>
                    <p className="text-xs text-on-surface-variant mt-0.5">Últimos 6 meses</p>
                  </div>
                  <div className="text-right">
                    <div className="text-2xl font-black text-primary">R$ 88.650</div>
                ...
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={revenueData}>
                      ...
                      <Tooltip
                        formatter={(value) => {
                          if (typeof value === "number") return [`R$ ${value.toLocaleString("pt-BR")}`, "Faturamento"];
                          if (typeof value === "string") return [`R$ ${value}`, "Faturamento"];
                          return ["—", "Faturamento"];
                        }}
                        contentStyle={{ borderRadius: 12, border: "1px solid hsl(var(--outline-variant))", fontSize: 13 }}
                      />
                      <Area type="monotone" dataKey="value" stroke="hsl(var(--primary))" strokeWidth={2.5} fill="url(#colorRevenue)" />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              </div>
              ...
              <div className="bg-surface-container-lowest border border-outline-variant/30 rounded-2xl p-6 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center gap-1.5 bg-secondary-container/10 px-3 py-1.5 rounded-lg">
                    ...
                    <span className="font-black text-on-secondary-container text-sm">4.6</span>
                ...
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={ratingsData}>
                      ...
                      <Tooltip
                        formatter={(value) => {
                          if (typeof value === "number") return [`${value.toFixed(1)} ★`, "Nota média"];
                          if (typeof value === "string") return [`${value} ★`, "Nota média"];
                          return ["—", "Nota média"];
                        }}
                        contentStyle={{ borderRadius: 12, border: "1px solid hsl(var(--outline-variant))", fontSize: 13 }}
                      />
              ...
              <div>
                <div className="flex justify-between items-center mb-4">
                  <div>
                    <h2 className="font-headline text-xl font-bold text-on-surface">Reservas recentes</h2>
                    <div className="h-0.5 w-12 bg-secondary-container mt-1" />
                  </div>
                  <button onClick={() => setTab("reservas")} className="text-sm font-bold text-primary hover:underline flex items-center gap-1">
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 bg-surface-container-high rounded-xl flex items-center justify-center">
                            <MaterialIcon icon="precision_manufacturing" className="text-primary" size={20} />
                          </div>
                          <div>
          ...
          {tab === "frota" ? (
            <div className="space-y-6">
              <div className="flex justify-between items-start">
                <div>
                  <h1 className="font-headline text-3xl font-bold text-primary">Minha Frota</h1>
                  <div className="h-1 w-16 bg-secondary-container mt-2" />
                  <p className="text-on-surface-variant text-sm mt-3">Gerencie seus equipamentos cadastrados</p>
                </div>
                <Link
                ...
                >
                  <div className="flex items-center gap-6">
                    <div className="w-14 h-14 bg-primary/10 rounded-xl flex items-center justify-center">
                      <MaterialIcon icon="agriculture" className="text-primary" size={28} />
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-4">
                    {m.status === "active" ? (
                      <span className="px-3 py-1.5 rounded text-[10px] font-bold uppercase tracking-wider bg-primary/10 text-primary border border-primary/20 flex items-center gap-1.5">
                        <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" /> Ativo
                      </span>
                    ) : (
                    ...
                    <button onClick={() => openEditModalForMachine(m)} className="text-sm font-bold text-primary hover:underline">
                      Editar
                    </button>
                  </div>
                </div>
              ))}
              <DashboardPagination currentPage={frotaPage} totalPages={frotaTotalPages} onPageChange={setFrotaPage} />
            </div>
          ) : null}
          ...
          {tab === "anuncios" ? (
            <div className="space-y-6">
              <div className="flex justify-between items-start">
                <div>
                  <h1 className="font-headline text-3xl font-bold text-primary">Anúncios</h1>
                  <div className="h-1 w-16 bg-secondary-container mt-2" />
                  <p className="text-on-surface-variant text-sm mt-3">Gerencie seus anúncios publicados</p>
                </div>
                <Link
                  ...
                  >
                    <div className="h-40 bg-gradient-to-br from-primary/10 via-secondary-container/10 to-tertiary/10 overflow-hidden flex items-center justify-center">
                      <div className="w-14 h-14 bg-surface-container-lowest/70 rounded-2xl flex items-center justify-center border border-outline-variant/20 backdrop-blur-sm">
                        <MaterialIcon icon="precision_manufacturing" className="text-primary" size={28} />
                      </div>
                    </div>
                    <div className="p-5">
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="font-headline font-bold text-on-surface">{p.machine}</h4>
                      ...
                      <p className="text-sm text-on-surface-variant mb-3 flex items-center gap-1">
                        <MaterialIcon icon="location_on" size={16} /> {p.location}
                      </p>
                      <div className="flex justify-between items-center">
                        <div className="text-xl font-black text-primary">
                          R$ {p.price}
                          <span className="text-sm font-bold text-tertiary">/h</span>
                        </div>
              ...
              <DashboardPagination currentPage={anunciosPage} totalPages={anunciosTotalPages} onPageChange={setAnunciosPage} />
            </div>
          ) : null}
          ...
          {tab === "reservas" ? (
            <div className="space-y-6">
              <div>
                <h1 className="font-headline text-3xl font-bold text-primary">Locações</h1>
                <div className="h-1 w-16 bg-secondary-container mt-2" />
              ...
              {activeRentals.length > 0 ? (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">{activeRentals.map(renderRentalCard)}</div>
                </div>
              ) : null}
              ...
              {pastRentals.length > 0 ? (
                <div className="space-y-4 mt-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">{pastRentals.map(renderRentalCard)}</div>
                </div>
              ) : null}
          ...
          {tab === "contratos" ? (
            <div className="space-y-6">
              <div>
                ...
                <div className="h-1 w-16 bg-secondary-container mt-2" />
                <p className="text-on-surface-variant text-sm mt-3">Gerencie seus contratos de locação</p>
              </div>
              <DashboardSearchBar
                searchValue=""
                onSearchChange={() => {}}
                ...
                {["Todos", "Pendentes", "Assinados", "Encerrados"].map((f) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg text-sm font-bold transition-colors ${
                      f === "Todos" ? "bg-primary text-on-primary" : "bg-surface-container text-on-surface-variant hover:bg-surface-container-high"
                    }`}
                  >
                    {f}
                  </button>
              ...
              {[
                { id: "#CTR-8821", lessee: "Fazenda Aurora", machine: "Trator Valtra BH194", period: "02 – 10 Fev/2026", total: "R$ 15.000,00", status: "pending", date: "28 Jan/2026" },
                { id: "#CTR-8799", lessee: "Fazenda São João", machine: "Colheitadeira JD S700", period: "15 – 20 Jan/2026", total: "R$ 38.400,00", status: "signed", date: "12 Jan/2026" },
                { id: "#CTR-8745", lessee: "Fazenda Boa Vista", machine: "Pulverizador Jacto", period: "01 – 05 Dez/2025", total: "R$ 6.250,00", status: "closed", date: "28 Nov/2025" },
              ].map((c) => (
                <div key={c.id} className="bg-surface-container-lowest border border-outline-variant/30 rounded-2xl p-6 hover:shadow-xl transition-all duration-300 shadow-sm">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-surface-container-low p-4 rounded-xl border border-outline-variant/20 mb-4">
                    <div>
                      <div className="text-[10px] font-bold text-outline uppercase tracking-wider mb-1">Locatário</div>
                      <div className="font-bold text-tertiary text-sm">{c.lessee}</div>
                    </div>
                    <div>
                      <div className="text-[10px] font-bold text-outline uppercase tracking-wider mb-1">Período</div>
          ...
          {tab === "chat" ? (
            <div className="space-y-6">
              <div>
                ...
                <div className="h-1 w-16 bg-secondary-container mt-2" />
                <p className="text-on-surface-variant text-sm mt-3">Converse com seus locatários</p>
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100vh-240px)]">
                <div className="bg-surface-container-lowest border border-outline-variant/30 rounded-2xl overflow-hidden flex flex-col shadow-sm">
                  ...
                  <div className="flex-1 overflow-y-auto">
                    {[
                      { name: "Fazenda Aurora", initials: "FA", lastMsg: "Vou confirmar a reserva então!", time: "10:32", unread: 2, online: true },
                      { name: "Fazenda São João", initials: "SJ", lastMsg: "Quando posso retirar a colheitadeira?", time: "Ontem", unread: 0, online: false },
                      { name: "Fazenda Boa Vista", initials: "BV", lastMsg: "Obrigado pelo excelente serviço!", time: "20/01", unread: 0, online: false },
                    ].map((contact, i) => (
                      <div
                        key={contact.name}
                ...
                <div className="lg:col-span-2 bg-surface-container-lowest border border-outline-variant/30 rounded-2xl overflow-hidden flex flex-col shadow-sm">
                  <div className="p-4 border-b border-outline-variant/30 bg-surface-container flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-bold text-on-surface text-sm">Fazenda Aurora</div>
                  ...
                  <div className="flex-1 p-6 space-y-4 overflow-y-auto bg-surface/50">
                    ...
                    <div className="flex flex-col items-start gap-1">
                      ...
                      <div className="bg-surface-container p-3.5 rounded-2xl rounded-tl-sm text-sm text-tertiary max-w-[75%] leading-relaxed font-medium shadow-sm">
                        Bom dia! O Trator Valtra BH194 está disponível para o período de 02 a 10 de fevereiro?
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-1">
                      ...
                      <div className="bg-primary text-on-primary p-3.5 rounded-2xl rounded-tr-sm text-sm max-w-[75%] leading-relaxed font-medium shadow-sm">
                        Bom dia! Sim, está disponível. Já com operador certificado NR-31.
                      </div>
                    </div>
                    <div className="flex flex-col items-start gap-1">
          ...
          {tab === "notificacoes" ? (
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                {[
                  { icon: "description", title: "Contrato aguardando assinatura", desc: "Fazenda Aurora · Trator Valtra BH194 · 02–10 Fev/2026", time: "Agora", unread: true },
                  { icon: "event_available", title: "Nova solicitação de reserva", desc: "Fazenda São João · Colheitadeira JD S700", time: "2h atrás", unread: true },
                  { icon: "star", title: "Nova avaliação recebida", desc: "Fazenda Boa Vista avaliou com 5 estrelas", time: "Ontem", unread: false },
                  { icon: "event_repeat", title: "Solicitação de reagendamento", desc: "Fazenda Aurora deseja alterar período para 15–20 Fev/2026", time: "Ontem", unread: false },
                  { icon: "payments", title: "Pagamento recebido", desc: "R$ 15.000,00 — Fazenda São João · Colheitadeira JD S700", time: "3 dias atrás", unread: false },
                ].map((n, i) => (
                  <div
                    key={i}
                    className={`flex items-start gap-4 p-5 rounded-2xl border transition-all hover:shadow-md ${
                      n.unread ? "bg-primary/5 border-primary/20" : "bg-surface-container-lowest border-outline-variant/30"
                    }`}
                  >
                    ...
                    {n.unread ? <span className="w-2.5 h-2.5 bg-primary rounded-full mt-1.5 shrink-0 animate-pulse" /> : null}
          ...
          {tab === "avaliacoes" ? (
            <div className="space-y-8">
              ...
              <div>
                ...
                <div className="space-y-4">
                  {[
                    { from: "Fazenda Boa Vista", initials: "BV", machine: "Pulverizador Jacto", date: "06/12/2025", rating: 5, comment: "Equipamento em ótimo estado, operador muito competente. Recomendo!" },
                    { from: "Fazenda São João", initials: "SJ", machine: "Colheitadeira JD S700", date: "21/01/2026", rating: 4, comment: "Colheitadeira funcionou perfeitamente. Apenas um pequeno atraso na entrega." },
                    { from: "Fazenda Aurora", initials: "FA", machine: "Trator Valtra BH194", date: "11/02/2026", rating: 5, comment: "Excelente trator, muito bem cuidado. João foi muito atencioso com tudo." },
                  ].map((r) => (
                    <div key={r.from + r.date} className="bg-surface-container-lowest border border-outline-variant/30 rounded-2xl p-6 hover:shadow-md transition-shadow shadow-sm">
                        ...
                        <div className="flex gap-0.5">
                          {Array.from({ length: 5 }).map((_, j) => (
                            <MaterialIcon
                              ...
                              icon="star"
                              filled={j < r.rating}
                              className={j < r.rating ? "text-secondary-container" : "text-outline/30"}
                              size={16}
                            />
                          ))}
                        </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  {[
                    { to: "Fazenda Boa Vista", initials: "BV", machine: "Pulverizador Jacto", date: "06/12/2025", rating: 5, comment: "Locatário muito cuidadoso com o equipamento. Devolveu tudo em perfeito estado." },
                    { to: "Fazenda São João", initials: "SJ", machine: "Colheitadeira JD S700", date: "21/01/2026", rating: 5, comment: "Excelente locatário, pagamento pontual e comunicação ótima." },
                  ].map((r) => (
                    <div key={r.to + r.date} className="bg-surface-container-lowest border border-outline-variant/30 rounded-2xl p-6 hover:shadow-md transition-shadow shadow-sm">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-3">
