...
import { Search, MapPin, Star, CalendarDays, SlidersHorizontal, X, SearchX, ChevronLeft, ChevronRight } from "lucide-react";
...
const anuncios = [
  { id: 1, imagem: "https://loremflickr.com/800/600/harvester,agriculture?lock=1", titulo: "Colheitadeira JD S700",            cidade: "Sorriso, MT",            preco: 480, atividade: "Colheita",        horas: "2.450 h", nota: 4.8, de: "01/03/2026", ate: "30/04/2026" },
  { id: 2, imagem: "https://loremflickr.com/800/600/tractor,farm?lock=2",          titulo: "Trator Valtra BH194",              cidade: "Lucas do Rio Verde, MT", preco: 320, atividade: "Preparo de Solo", horas: "1.800 h", nota: 4.5, de: "15/02/2026", ate: "20/05/2026" },
  { id: 3, imagem: "https://loremflickr.com/800/600/sprayer,agriculture?lock=3",   titulo: "Pulverizador Jacto Uniport",       cidade: "Rondonópolis, MT",       preco: 250, atividade: "Pulverização",    horas: "950 h",   nota: 4.9, de: "01/02/2026", ate: "30/06/2026" },
  { id: 4, imagem: "https://loremflickr.com/800/600/harvester,field?lock=4",       titulo: "Colheitadeira NH CR 7.90",         cidade: "Primavera do Leste, MT", preco: 520, atividade: "Colheita",        horas: "3.100 h", nota: 4.6, de: "10/03/2026", ate: "15/05/2026" },
  { id: 5, imagem: "https://loremflickr.com/800/600/tractor,agriculture?lock=5",   titulo: "Trator MF 8737",                   cidade: "Campo Verde, MT",        preco: 380, atividade: "Plantio",         horas: "2.200 h", nota: 4.7, de: "01/09/2026", ate: "30/11/2026" },
  { id: 6, imagem: "https://loremflickr.com/800/600/sprayer,field?lock=6",         titulo: "Pulverizador Stara Imperador 3.0", cidade: "Sinop, MT",              preco: 290, atividade: "Pulverização",    horas: "1.050 h", nota: 4.4, de: "01/01/2026", ate: "31/12/2026" },
  { id: 7, imagem: "https://loremflickr.com/800/600/harvester,grain?lock=7",       titulo: "Colheitadeira Case IH 8250",       cidade: "Sapezal, MT",            preco: 510, atividade: "Colheita",        horas: "2.800 h", nota: 4.7, de: "01/03/2026", ate: "31/05/2026" },
  { id: 8, imagem: "https://loremflickr.com/800/600/tractor,field?lock=8",         titulo: "Trator John Deere 7215R",          cidade: "Querência, MT",          preco: 410, atividade: "Plantio",         horas: "1.600 h", nota: 4.6, de: "01/08/2026", ate: "30/11/2026" },
  { id: 9, imagem: "https://loremflickr.com/800/600/sprayer,crop?lock=9",          titulo: "Pulverizador Montana 4000",        cidade: "Água Boa, MT",           preco: 270, atividade: "Pulverização",    horas: "880 h",   nota: 4.3, de: "01/01/2026", ate: "31/12/2026" },
];
...
const Buscar = () => {
  ...
  const resultados = anuncios
    .filter((a) => {
      ...
      if (dataInicio && a.ate < dataInicio.split("-").reverse().join("/"))            return false;
      if (dataFim    && a.de  > dataFim.split("-").reverse().join("/"))               return false;
      return true;
    })
  ...
  const itensPaginados = lista.slice((pagina - 1) * ITENS_POR_PAGINA, pagina * ITENS_POR_PAGINA);
      ...
      <div className="flex-1 pt-32 pb-20 max-w-[1200px] mx-auto px-6 w-full">
        ...
        <div className="bg-white rounded-[24px] border border-gray-100 p-8 mb-10 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div className="md:col-span-3 space-y-2">
              ...
              <Input placeholder="Ex: Trator, Colheitadeira..." value={busca} onChange={(e) => setBusca(e.target.value)} />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input placeholder="Ex: Sorriso, MT" value={cidade} onChange={(e) => setCidade(e.target.value)} />
            </div>

            <div className="space-y-2">
              <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Preço máx. (R$/h)</label>
              <Input type="number" placeholder="Ex: 400" min={0} value={precoMax} onChange={(e) => setPrecoMax(e.target.value)} />
            </div>
        ...
        {itensPaginados.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {itensPaginados.map((a) => (
              <div key={a.id} className="bg-white border border-gray-100 rounded-[20px] overflow-hidden group shadow-sm hover:shadow-xl transition-all duration-300">
                <div className="h-52 relative overflow-hidden bg-gray-100">
                  <img src={a.imagem} alt={a.titulo} loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                ...
                <div className="p-6 flex flex-col gap-3">
                  <div className="flex justify-between items-start gap-2 min-h-[48px]">
                    <h4 className="font-bold text-[#1A2414] text-base leading-tight">{a.titulo}</h4>
                    <span className="text-xs font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded flex items-center gap-1 shrink-0">
                      <Star className="w-3 h-3 fill-amber-500 text-amber-500" /> {a.nota}
                    </span>
                  </div>
                  ...
                  <div className="border-t border-gray-100 pt-3 flex flex-col gap-1.5 text-xs">
                    <div className="flex gap-2">
                      <span className="bg-[#F3F0E6] text-[#2D3F1E] font-bold px-2.5 py-1 rounded-lg">{a.atividade}</span>
                      <span className="bg-gray-50 text-gray-500 font-medium px-2.5 py-1 rounded-lg">{a.horas}</span>
                    </div>
                    <span className="bg-gray-50 text-gray-500 font-medium px-2.5 py-1 rounded-lg w-fit">{a.de} – {a.ate}</span>
                  </div>

                  <div className="flex justify-between items-center pt-1">
                    <div className="text-2xl font-black text-[#2D3F1E]">
                      R$ {a.preco}<span className="text-sm font-bold text-gray-400">/h</span>
                    </div>
                    <Link to={`/anuncio/${a.id}`} className="bg-[#2D3F1E] text-white px-5 py-2.5 rounded-xl font-bold text-sm hover:bg-[#1A2414] transition-colors">
                      Ver Detalhes
                    </Link>
                  </div>
                </div>
        ...
        {lista.length > 0 && (
          <div className="flex items-center justify-center gap-2 mt-12">
            ...
            <button onClick={() => setPagina((p) => Math.min(totalPgs, p + 1))} disabled={pagina === totalPgs}
              className="w-9 h-9 flex items-center justify-center rounded-xl border border-gray-200 bg-white text-[#2D3F1E] hover:bg-[#F3F0E6] disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
