...
import type { ChatMessage, ChatThread, UnreadCounts } from "../types/chat";
import {
      ...
      switch (data.type) {
        case "message.new":
          h.onMessage?.(
            ...
            toChatMessage(chatMessageApiSchema.parse(data.message)),
          );
          break;
        case "message.read":
          h.onRead?.(
        ...
        case "unread.updated":
          h.onUnread?.(
            toUnreadCounts(
              unreadCountsApiSchema.parse({
                unread_total: data.unread_total,
                unread_threads: data.unread_threads,
              }),
            ),
          );
          break;
        case "thread.updated":
          h.onThreadUpdated?.(toChatThread(chatThreadApiSchema.parse(data.thread)));
          break;
        default:
          break;
      }
